...
class smb {
    ...
    function execute ($command, $purl, $hide_errors = false) {
    ...
    function client ($params, $purl, $hide_errors = false) {

        static $regexp = array (
        '.*Error NT_STATUS_(.*)\).*' => 'error',
        ...
        $output = popen ("LANG=\"fr_FR.UTF-8\" " . SMB4PHP_SMBCLIENT." -N {$auth} {$options} {$port} {$options} {$params} 2>/dev/null", 'r');
        //echo "LANG=\"fr_FR.UTF-8\" " . SMB4PHP_SMBCLIENT." -N {$auth} {$options} {$port} {$options} {$params} 2>/dev/null", 'r';
        $info = array ();
    ...
    function url_stat ($url, $flags = STREAM_URL_STAT_LINK) {
        ...
        switch ($pu['type']) {
            ...
            case 'share':
                ...
                break;
            case 'path':
                if ($o = smb::execute ('dir "'.$pu['path'].'"', $pu, true)) {
                    $p = split ("[\\]", $pu['path']);
                    $name = $p[count($p)-1];
    ...
    public static function downlod_file($url) {
        $handle = popen("smbget " . escapeshellarg($url) . " -O", "r");
        if ($handle) {
            while(!feof($handle))
            {
        ...
        switch ($pu['type']) {
            ...
            case 'path':
                if ($o = smb::execute ('dir "'.$pu['path'].'\*"', $pu)) {
                   ...
                   if($o['error']) new wizz("SMB4PHP a renvoyé l'erreur suivante : <br />" . "dir_opendir(): dir failed for path '{$pu['path']}'", WIZZ_ERROR, $objResponse);
                }
                break;
            default:
    ...
    function stream_open ($url, $mode, $options, $opened_path) {
        ...
        switch ($mode) {
            ...
            case 'a+':  $this->tmpfile = tempnam('/tmp', 'smb.down.');
                        $e = smb::execute ('get "'.$pu['path'].'" "'.$this->tmpfile.'"', $pu, true);
