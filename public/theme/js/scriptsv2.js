...
$(function() {
    ...
    $(".taglist").each(function() {
        $(this).select2({tags: $(this).data('availableTags'), maximumSelectionSize: $(this).data('maxSelectableTags')});
    });
...
var igestisWizz = function(msg, type, target, preserveWizzs) {
    ...
    if (target !== undefined && target !== null) {
        ...
        if (!preserveWizzs)
            $("#general-container > .alert").remove();
    ...
    if (jsonData.IgestisAjaxReplace !== undefined && $.isArray(jsonData.IgestisAjaxReplace)) {
        for (i = 0; i < jsonData.IgestisAjaxReplace.length; i++) {
            ...
            if ($field.is("textarea") || $field.is("input")) {
                ...
                $field.html(jsonData.IgestisAjaxReplace[i].value);
        }
    }
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result === null) {
                    ...
                    $(".taglist").each(function() {
                        $(this).select2({tags: $(this).data('availableTags')});
                    });
                    avtiveRadioButtons();
